        ...
        # если кнопок много, то просим выбрать что-то одно
            ...
            for i in range(lenght):
                button1 = InlineKeyboardButton(
                    text=f"{list_schedule[i*2]}",
                    callback_data=f"Выбор{list_schedule[i*2]}",
                )
                button2 = InlineKeyboardButton(
                    text=f"{list_schedule[i*2+1]}",
                    callback_data=f"Выбор{list_schedule[i*2+1]}",
                )
            ...
            if len(list_schedule) % 2 == 1:
                button = InlineKeyboardButton(
                    text=f"{list_schedule[-1]}",
                    callback_data=f"Выбор{list_schedule[-1]}",
                )
                list_keyboard_buttons.append([button])
            keyboard = InlineKeyboardMarkup(inline_keyboard=list_keyboard_buttons)
            await message.answer(
                "Выберите группу/преподавателя/аудиторию", reply_markup=keyboard
            )
...
@router_user.callback_query(
    F.data.startswith("Сегод") | F.data.startswith("Завтр") | F.data.startswith("После")
)
...
async def find_week(callback: CallbackQuery):
    ...
    # если ошибка, то отправляем текст, а потом новую клавиатуру
        ...
        await callback.message.answer(
            f"Фильтры расписания для {callback.data[16:]}", reply_markup=keyboard
        )
    await callback.answer()
...
async def find_all(callback: CallbackQuery):
    ...
    # если ошибка, то отправляем текст, а потом новую клавиатуру
        ...
        await callback.message.answer(text)
        await callback.message.answer(
            f"Фильтры расписания для {callback.data[23:]}", reply_markup=keyboard
        )
    await callback.answer()