...
from dbrequests import (
    ...
    get_list_teachers,
    get_list_cabinet,
    get_all_date_for_valid_text,
    get_all_data_from_date,
)
import datetime
...
async def get_list_schedule(text):
    ...
    for teacher in list_teachers_from_db:
        if (
            text in teacher.split()[0].strip().lower()
            or text in teacher.strip().lower()
        ):
    ...
    if text.find("-") > 0:
        ...
        input_split_dash = [
            value.strip() for value in input_split_dash if value != "" and value != " "
        ]
    ...
    for valid_value_from_db in list_groups_from_db:
        for value_for_check in list_to_check:
            if (
                value_for_check in valid_value_from_db.lower()
                and valid_value_from_db not in list_valid_data_for_buttons
            ):
                # если группа с номером подгруппы
                if len(valid_value_from_db.split()) > 1:
                    list_valid_data_for_buttons.append(
                        valid_value_from_db.split()[0][:-1]
                    )
                else:
...
async def get_schedule_by_date(valid_text, period):
    ...
    if period == "Сегод":
        ...
        list_date.append(
            datetime.datetime.today() + datetime.timedelta(days=1, hours=3)
        )
    elif period == "После":
        list_date.append(
            datetime.datetime.today() + datetime.timedelta(days=2, hours=3)
        )

    # если другой период
    elif "Неделя" in period:
        begin_week: str = period[6:16]
        begin_week = datetime.datetime(
            int(begin_week[:4]), int(begin_week[5:7]), int(begin_week[8:10])
        )
        ...
        begin_week = datetime.datetime(
            int(begin[:4]), int(begin[5:7]), int(begin[8:10])
        )
    ...
    for date in list_date:
        ...
        if len(all_data_from_date) > 0:
            ...
            for i in range(len(all_data_from_date)):
                ...
                date_pair = datetime.datetime(
                    int(date_pair[6:]) + 2000, int(date_pair[3:5]), int(date_pair[:2])
                )
...
async def get_keyboard_and_text(valid_text, period=None):
    ...
    button3 = InlineKeyboardButton(
        text="Послезавтра", callback_data=f"После{valid_text}"
    )
    ...
    first_day_week = current_date - datetime.timedelta(
        days=current_date.date().weekday()
    )
    ...
    while first_day_week < last_date:
        date_current_week = [
            first_day_week + datetime.timedelta(days=i) for i in range(7)
        ]
    ...
    last_day_week = (
        last_date
        - datetime.timedelta(days=last_date.date().weekday())
